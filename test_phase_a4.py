...
1. The already-frozen 2026 Dutch GP snapshot (real grid/form/track-history,
   ...
   market comparison should be attempted -- and this is also where the real
   fastest-lap ingest-lag gotcha (sec7.4) actually reproduces, not a
   hypothetical.
...
2. A fresh snapshot built for the archived 2023 Dutch GP (round looked up
   ...
   already uses) -- fully resolved including fastest lap (verified live: 2023
   data has FastestLap populated, unlike round 12 2026's ingest lag), so
   this is the one place the fastest-lap outcome path gets exercised
   end to end against a real, known answer (Alonso, P2, despite Verstappen
   winning -- confirms fastest lap is independent of finishing position).
"""
...
import score
from postrace import find_full_result, find_fastest_lap, FastestLapNotIngestedError
...
class TestDutchGP2026OutcomeOnly(unittest.TestCase):
    ...
    def test_fastest_lap_not_yet_ingested_for_this_round(self):
        """The real, verified ingest-lag case (sec7.4) -- not simulated."""
        with self.assertRaises(FastestLapNotIngestedError):
            find_fastest_lap(self.rows)
