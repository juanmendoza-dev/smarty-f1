...
def discover_event_ticker(series_ticker, cache_dir):
    url = f"{BASE}/events?" + urllib.parse.urlencode({"series_ticker": series_ticker, "status": "open"})
    body, meta = httpcache.cached_get_json(url, cache_dir)
    events = body.get("events", [])
    return events, meta


def get_markets(event_ticker, cache_dir):
...
def resolve_markets(series_ticker, event_ticker, expected_race_date, cache_dir):
    ...
    """
    events, events_meta = discover_event_ticker(series_ticker, cache_dir)
    ...
    body, markets_meta = get_markets(event_ticker, cache_dir)
