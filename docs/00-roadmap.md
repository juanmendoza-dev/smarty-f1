...
- Lane B tick client scope (`03` §7): parse the feed down to per-car position, gap/interval, in-pit/retired, speed/throttle/brake/gear/rpm, and X/Y — keyed on the FIA three-letter code, same canonical key as Lane A (`01` §8.2). Twelve channels subscribed, not the ~30 the feed carries; media, weather, and presentational channels deliberately left out. Channel 45 (DRS pre-2026, active aero now) is carried as an **opaque integer** — its 2026 encoding is unconfirmed and B0 does not guess at it
