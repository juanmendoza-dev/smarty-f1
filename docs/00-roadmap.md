...
Status: **snapshot + scorer built, reference prediction verified** — see `02-winner-prediction-algo.md` §9. Algo: NOR 36.2%, RUS 35.2%, ANT 11.4%. Market: NOR 37.2%, RUS 25.0%, ANT 25.0%. Two testable divergences (RUS +10.2, ANT -13.6). A real first snapshot is committed in `data/snapshots/`. Remaining: re-snapshot the markets close to lights-out (13:00 UTC, 2026-08-23 — owner triggers manually), and record the actual result + Brier scores after the race — needs a small `postrace.py`, not yet built, since `score.py` only computes Brier scores if you manually pass `--winner CODE`.
