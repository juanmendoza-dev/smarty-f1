...
Status: **specced 2026-08-27; approved 2026-09-03; BUILT AND VALIDATED 2026-09-04.** The offline layer meets `09` §10's pre-registered success criteria — see `09` §10.1 for the run, and read §10.2/§10.4 with it, because two defects are reported rather than fixed.

**What the run found**, in one block, because a status line that only carries the good half is not a status line:
- **The layer succeeds.** Pooled log-loss 0.799 against the position-only ladder's 1.000 and static Lane A's 1.177, over 8 races and 520 checkpoints; it beats static in 8 of 8 races and the ladder in 7 of 8, against a pre-registered bar of 6.
- **But the interval crosses zero on the comparison that matters.** Block-bootstrapped over whole races, layer − ladder is −0.176 with a 95% CI of [−0.438, **+0.226**]. Eight races cannot size that gap, exactly as `09` §9.3 pre-registered.
- **The one loss is the race where Lane A was wrong.** R9 (British): the prior gave the winner ~3%, strengths are frozen for the race (`09` §6), and the layer loses to the ladder by 1.13. That is `09` §1.3 happening on schedule and it is the first real evidence for `09` §13 item 1.
- **`08` earns its place by the letter of the rule and by very little else.** layer − ablation = **−0.00064** log-loss, 95% CI [−0.00145, −0.00001] under common random numbers — real, correctly signed, and **0.4% of the layer's own margin over the ladder**, carried by two races of eight. `09` §3 predicted "real and small" and that is what it is.
- **The layer is silent on 28.5% of checkpoints** from `09` §5.7's pit rule (33.3% unreliable overall). `09` §5.7 pre-registered that landing near 34.5% would be a headline; it did.
- **`09` §11 assertion 3 fails**: filtered as the spec defines it (≤5 laps remaining, green flag), the leader's `p_win` sits below the 0.9 band on **16 of 42 checkpoints**, minimum 0.714. Traced to one cell — §5.4 hands the P1/P2 pair the pooled P1–P3 band rate, and in the closing quarter the simulator disperses that pair **9.9× faster than the archive** (`09` §10.2). Not patched: reconditioning after seeing the validation result is the `05` §6.4.1 failure. `09` §13 item 6.
- **A claim that did not survive checking, recorded because this project records them.** An earlier draft of `09` §10.2 said the simulator over-disperses the whole field by 1.6×, inferred from archive swap counts. Measured directly against the real simulator it tracks the archive at **0.99 pooled** — the shrinkage and the strength tilt absorb it. The 0.61 figure is a property of `09` §5.4's raw rate, not of the layer. `09` §16.6 item 7 has how the wrong version got written.

Six measurements were run against the 12-race archive before the spec was written, and two of them reshape the lane:
