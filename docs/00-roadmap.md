...
- **It over-disperses the field by a measured 39%** — net displacement at five laps is 0.61× what `09` §5.4's per-lap swap rate compounded predicts, in every band and every quarter — which is why the position ladder beats it in the last two progress deciles. Not patched: fixing it after seeing the validation result is the `05` §6.4.1 failure. `09` §13 item 6 and `docs/12`.
